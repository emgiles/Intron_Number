#### Script for generating a plot of number of exons by percent of transcripts for multiple genome comparisons
...
#collect introns by transcript
introns.list.per.transcript <- intronsByTranscript(S.viridula.p, use.names=TRUE)
mcols(introns.list.per.transcript) #get information about data collected
head(introns.list.per.transcript)

#coerce to dataframe
introns.list.per.transcript.df.sv.all <- as.data.frame(introns.list.per.transcript)
head(introns.list.per.transcript.df.sv.all, n=50)
colnames(introns.list.per.transcript.df.sv.all) <- c("GENEID", "TXID","CHRMID","start","end","width","strand")
head(introns.list.per.transcript.df.sv.all)

#determine number of introns per transcript
mcols(introns.list.per.transcript)$num_introns <- lengths(introns.list.per.transcript)
introns_per_transcript.sv <- as.data.frame(mcols(introns.list.per.transcript))
introns_per_transcript.sv$TXID <- row.names(introns_per_transcript.sv)
head(introns_per_transcript.sv)
class(introns_per_transcript.sv)

#merge based on TXID
merged.sv <- dplyr::inner_join(introns_per_transcript.sv, introns.list.per.transcript.df.sv.all, by = "TXID")
head(merged.sv)

#subset only the columns that are needed
merged.sv.reduced <- subset(merged.sv, select=c(num_introns, TXID, GENEID, CHRMID))
...
#collect introns by transcript
introns.list.per.transcript <- intronsByTranscript(S.zebrina.p, use.names=TRUE)
mcols(introns.list.per.transcript) #get information about data collected
head(introns.list.per.transcript)

#coerce to dataframe
introns.list.per.transcript.df.sz.all <- as.data.frame(introns.list.per.transcript)
head(introns.list.per.transcript.df.sz.all, n=50)
colnames(introns.list.per.transcript.df.sz.all) <- c("GENEID", "TXID","CHRMID","start","end","width","strand")
head(introns.list.per.transcript.df.sz.all)

#determine number of introns per transcript
mcols(introns.list.per.transcript)$num_introns <- lengths(introns.list.per.transcript)
introns_per_transcript.sz <- as.data.frame(mcols(introns.list.per.transcript))
introns_per_transcript.sz$TXID <- row.names(introns_per_transcript.sz)
head(introns_per_transcript.sz)
class(introns_per_transcript.sz)

#merge based on TXID
merged.sz <- dplyr::inner_join(introns_per_transcript.sz, introns.list.per.transcript.df.sz.all, by = "TXID")
head(merged.sz)

#subset only the columns that are needed
merged.sz.reduced <- subset(merged.sz, select=c(num_introns, TXID, GENEID, CHRMID))
...
boxplot(num_introns ~ ORTHGRP,
        data = merged_df)

#Kruskal Wallis tests
kruskal.test(num_introns ~ spID,
             data = merged_df)

pairwise.wilcox.test(merged_df$num_introns, merged_df$spID,
                     p.adjust.method = "fdr")
